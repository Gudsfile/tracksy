...
test("has title and can upload dataset", async ({ page }) => {
  ...
  await fileInput.setInputFiles(path.join(__dirname, "../datasets/streamings_1000.zip"));
  ...
  await expect(page.getByRole("listitem").filter({ hasText: "Top 5" })).toContainText("31.1%");

  await expect(page.getByRole("listitem").filter({ hasText: "Top 10" })).toContainText("44.6%");

  await expect(page.getByRole("listitem").filter({ hasText: "Top 20" })).toContainText("63.5%");
  ...
  await expect(loyaltyCard.getByRole("listitem").filter({ hasText: "Artists" })).toContainText("47");
  await expect(loyaltyCard.getByRole("listitem").filter({ hasText: "Streams" })).toContainText("74");

  /* Listening Rhythm Card */
  await expect(page.getByRole("heading", { name: /Listening Rhythm/ })).toBeVisible();
  await expect(page.getByRole("listitem").filter({ hasText: "Morning" })).toContainText("23.0%");
  await expect(page.getByRole("listitem").filter({ hasText: "Night" })).toContainText("39.2%");
  ...
  await expect(page.getByRole("listitem").filter({ hasText: "Total streams" })).toContainText("778");
  await expect(page.getByRole("listitem").filter({ hasText: "This year" })).toContainText("74");
  ...
  await expect(page.getByRole("listitem").filter({ hasText: "Fall" })).toContainText("33.8%");

  /* New vs Old Card */
  await expect(page.getByRole("heading", { name: /New vs Old/ })).toBeVisible();
  await expect(page.getByText("5 new artists discovered this year!")).toBeVisible();
  await expect(page.getByRole("listitem").filter({ hasText: "Découvertes" })).toContainText("14%");

  /* Listening Patience Card */
  await expect(page.getByRole("heading", { name: /Listening Patience/ })).toBeVisible();
  await expect(page.getByText("100.0%")).toBeVisible();
  await expect(page.getByRole("listitem").filter({ hasText: "Completed" })).toContainText("74");
  ...
  await expect(page.getByRole("listitem").filter({ hasText: "Windows" })).toContainText("45.9%");
