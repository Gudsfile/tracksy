...
Table ${f}: aggregate stats cache (rarely needed).
...
`;function O(){return Object.keys(u).map(a=>{const n=u[a],t=[];n.acceptsYear&&t.push("year (int, optional)"),n.acceptsLimit&&t.push("limit (int, optional)");const e=t.length?` [params: ${t.join(", ")}]`:"";return`  - ${a}${e}: ${n.description}`}).join(`
...
- Today is ${S}. When the user says "this year", "current year", or similar without an explicit year, use ${l} in the SQL and params.
`,R=[{user:"Who are my top artists?",assistant:JSON.stringify({intent:"top_artists",params:{},title:"Top artists",explanation:"Artists ranked by total stream count.",sql:"SELECT artist_name, COUNT(*)::DOUBLE AS count_streams, SUM(ms_played)::DOUBLE AS ms_played FROM music_streams WHERE artist_name IS NOT NULL GROUP BY artist_name ORDER BY count_streams DESC LIMIT 5"})},{user:"What are my top artists this year?",assistant:JSON.stringify({intent:"top_artists",params:{year:l},title:`Top artists ${l}`,explanation:`Artists ranked by total stream count in ${l}.`,sql:`SELECT artist_name, COUNT(*)::DOUBLE AS count_streams, SUM(ms_played)::DOUBLE AS ms_played FROM music_streams WHERE artist_name IS NOT NULL AND EXTRACT(year FROM ts) = ${l} GROUP BY artist_name ORDER BY count_streams DESC LIMIT 5`})},{user:"Show me my top 10 tracks in 2023",assistant:JSON.stringify({intent:"top_tracks",params:{year:2023,limit:10},title:"Top 10 tracks in 2023",explanation:"Most-played tracks during 2023.",sql:"SELECT track_name, artist_name, COUNT(*)::DOUBLE AS count_streams FROM music_streams WHERE EXTRACT(year FROM ts) = 2023 AND track_name IS NOT NULL GROUP BY track_name, artist_name ORDER BY count_streams DESC LIMIT 10"})},{user:"How does my listening look across the year?",assistant:JSON.stringify({intent:"calendar_heatmap",params:{year:new Date().getFullYear()},title:"Listening calendar",explanation:"Daily listening intensity across the calendar year.",sql:`SELECT ts::date AS day, COUNT(*)::DOUBLE AS stream_count FROM music_streams WHERE EXTRACT(year FROM ts) = ${new Date().getFullYear()} GROUP BY ts::date ORDER BY day`})},{user:"Streams per month for 2022",assistant:JSON.stringify({intent:"streams_per_month",params:{year:2022},title:"Streams per month — 2022",explanation:"Monthly stream counts for the year 2022.",sql:"SELECT DATE_TRUNC('month', ts) AS month, COUNT(*)::DOUBLE AS count_streams, SUM(ms_played)::DOUBLE AS ms_played FROM music_streams WHERE EXTRACT(year FROM ts) = 2022 GROUP BY month ORDER BY month"})},{user:"How many minutes per platform did I listen on?",assistant:JSON.stringify({intent:"custom",params:{},title:"Minutes listened per platform",explanation:"Total minutes of playback grouped by reported platform.",sql:"SELECT platform, SUM(ms_played) / 60000.0 AS minutes FROM music_streams WHERE platform IS NOT NULL GROUP BY platform ORDER BY minutes DESC"})},{user:"What is my skip rate?",assistant:JSON.stringify({intent:"skip_rate",params:{},title:"Skip rate",explanation:"Share of streams that were skipped vs completed.",sql:"SELECT COUNT(*) FILTER (WHERE ms_played < 30000)::DOUBLE AS skipped_listens, COUNT(*) FILTER (WHERE ms_played >= 30000)::DOUBLE AS complete_listens FROM music_streams"})}];class o extends Error{constructor(n,t){super(n),this.kind=t,this.name="LLMError"}kind}function N(a,n){const t=[{role:"system",content:g}];for(const s of R)t.push({role:"user",content:s.user}),t.push({role:"assistant",content:s.assistant});const e=n.slice(-8);for(const s of e)t.push({role:s.role,content:s.text});return t.push({role:"user",content:a}),t}function w(a){const n=a.match(/```(?:json)?\s*([\s\S]*?)\s*```/i),t=n?n[1]:a,e=t.indexOf("{");if(e===-1)throw new o("No JSON object found in model output.","parse");let s=0,r=!1,i=!1;for(let c=e;c<t.length;c++){const m=t[c];if(i){i=!1;continue}if(m==="\\"){i=!0;continue}if(m==='"'){r=!r;continue}if(!r){if(m==="{")s++;else if(m==="}"&&(s--,s===0))return t.slice(e,c+1)}}throw new o("Unbalanced JSON braces in model output.","parse")}function A(a){const n=w(a);let t;try{t=JSON.parse(n)}catch{throw new o("Model output is not valid JSON.","parse")}if(typeof t!="object"||t===null)throw new o("Model output is not a JSON object.","schema");const e=t;if(!T(e.intent))throw new o(`Unknown or missing intent: ${String(e.intent)}`,"schema");const s=e.intent,r=e.params??{},i={};typeof r.year=="number"&&Number.isFinite(r.year)&&(i.year=Math.trunc(r.year)),typeof r.limit=="number"&&Number.isFinite(r.limit)&&(i.limit=Math.trunc(r.limit));const c=typeof e.title=="string"&&e.title.trim().length>0?e.title.trim():"Result",m=typeof e.explanation=="string"?e.explanation.trim():"",p=typeof e.sql=="string"?e.sql.trim():"";if(p.length===0)throw new o("Response is missing the required sql field.","schema");return{intent:s,params:i,title:c,explanation:m,sql:p}}async function M(a,n,t){const e=N(n,t);let s;try{s=await a.chat.completions.create({messages:e,temperature:.1,max_tokens:512})}catch(r){const i=r instanceof Error?r.message:String(r);throw new o(i,"engine")}if(!("choices"in s)||!s.choices?.length||!s.choices[0].message?.content)throw new o("Empty response from model.","engine");return A(s.choices[0].message.content)}export{M as askLLM,w as extractJsonObject,A as parseChatAnswer};
