...
`;function y(){return Object.keys(p).map(a=>{const n=p[a],t=[];n.acceptsYear&&t.push("year (int, optional)"),n.acceptsLimit&&t.push("limit (int, optional)");const e=t.length?` [params: ${t.join(", ")}]`:"";return`  - ${a}${e}: ${n.description}`}).join(`
`)}const h=`You are a routing assistant for a local music-listening data app.
...
`,_=[{user:"Who are my top artists?",assistant:JSON.stringify({intent:"top_artists",params:{},title:"Top artists",explanation:"Artists ranked by total stream count."})},{user:"Show me my top 10 tracks in 2023",assistant:JSON.stringify({intent:"top_tracks",params:{year:2023,limit:10},title:"Top 10 tracks in 2023",explanation:"Most-played tracks during 2023."})},{user:"How does my listening look across the year?",assistant:JSON.stringify({intent:"calendar_heatmap",params:{year:new Date().getFullYear()},title:"Listening calendar",explanation:"Daily listening intensity across the calendar year."})},{user:"Streams per month for 2022",assistant:JSON.stringify({intent:"streams_per_month",params:{year:2022},title:"Streams per month — 2022",explanation:"Monthly stream counts for the year 2022."})},{user:"How many minutes per platform did I listen on?",assistant:JSON.stringify({intent:"custom",params:{},title:"Minutes listened per platform",explanation:"Total minutes of playback grouped by reported platform.",sql:"SELECT platform, SUM(ms_played) / 60000.0 AS minutes FROM music_streams WHERE platform IS NOT NULL GROUP BY platform ORDER BY minutes DESC"})},{user:"What is my skip rate?",assistant:JSON.stringify({intent:"skip_rate",params:{},title:"Skip rate",explanation:"Share of streams that were skipped vs completed."})}];class o extends Error{constructor(n,t){super(n),this.kind=t,this.name="LLMError"}kind}function g(a,n){const t=[{role:"system",content:h}];for(const s of _)t.push({role:"user",content:s.user}),t.push({role:"assistant",content:s.assistant});const e=n.slice(-8);for(const s of e)t.push({role:s.role,content:s.text});return t.push({role:"user",content:a}),t}function T(a){const n=a.match(/```(?:json)?\s*([\s\S]*?)\s*```/i),t=n?n[1]:a,e=t.indexOf("{");if(e===-1)throw new o("No JSON object found in model output.","parse");let s=0,i=!1,r=!1;for(let c=e;c<t.length;c++){const l=t[c];if(r){r=!1;continue}if(l==="\\"){r=!0;continue}if(l==='"'){i=!i;continue}if(!i){if(l==="{")s++;else if(l==="}"&&(s--,s===0))return t.slice(e,c+1)}}throw new o("Unbalanced JSON braces in model output.","parse")}function S(a){const n=T(a);let t;try{t=JSON.parse(n)}catch{throw new o("Model output is not valid JSON.","parse")}if(typeof t!="object"||t===null)throw new o("Model output is not a JSON object.","schema");const e=t;if(!f(e.intent))throw new o(`Unknown or missing intent: ${String(e.intent)}`,"schema");const s=e.intent,i=e.params??{},r={};typeof i.year=="number"&&Number.isFinite(i.year)&&(r.year=Math.trunc(i.year)),typeof i.limit=="number"&&Number.isFinite(i.limit)&&(r.limit=Math.trunc(i.limit));const c=typeof e.title=="string"&&e.title.trim().length>0?e.title.trim():"Result",l=typeof e.explanation=="string"?e.explanation.trim():"",m=typeof e.sql=="string"?e.sql:void 0;if(s==="custom"&&(!m||m.trim().length===0))throw new o("Custom intent requires a non-empty SQL string.","schema");return{intent:s,params:r,title:c,explanation:l,sql:m}}async function E(a,n,t){const e=g(n,t);let s;try{s=await a.chat.completions.create({messages:e,temperature:.1,max_tokens:512})}catch(i){const r=i instanceof Error?i.message:String(i);throw new o(r,"engine")}if(!("choices"in s)||!s.choices?.length||!s.choices[0].message?.content)throw new o("Empty response from model.","engine");return S(s.choices[0].message.content)}export{E as askLLM,T as extractJsonObject,S as parseChatAnswer};
