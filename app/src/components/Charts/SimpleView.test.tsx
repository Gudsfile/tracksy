...
it('renders all SimpleView', async () => {
    vi.spyOn(db, 'queryDBAsJSON').mockImplementation((query) => {
        ...
        if (query === queryTopTracks(2024).sql)
            return Promise.resolve(topTracksMock)
        if (query === queryTopArtists(2024).sql)
            return Promise.resolve(topArtistsMock)
        if (query === queryTopAlbums(2024).sql)
            return Promise.resolve(topAlbumsMock)
        if (query === queryConcentrationScore(2024).sql)
            return Promise.resolve(concentrationMock)
        if (query === queryArtistLoyalty(2024).sql)
            return Promise.resolve(artistReplayMock)
        if (query === queryListeningRhythm(2024).sql)
            return Promise.resolve(listeningRhythmMock)
        if (query === queryRegularity(2024).sql)
            return Promise.resolve(regularityMock)
        if (query === queryEvolutionOverTime())
            return Promise.resolve(evolutionOverTimeMock)
        if (query === querySeasonalPatterns(2024).sql)
            return Promise.resolve(seasonalPatternsMock)
        if (query === queryNewVsOld(2024).sql)
            return Promise.resolve(newVsOldMock)
        if (query === querySkipRate(2024).sql)
            return Promise.resolve(skipRateMock)
        if (query === queryRepeatBehavior(2024).sql)
            return Promise.resolve(repeatBehaviorMock)
        if (query === queryPrincipalPlatform(2024).sql)
            return Promise.resolve(principalPlatformMock)
        if (query === queryFavoriteWeekday(2024).sql)
    ...
    await screen.findByRole('heading', { name: '📈Soundtrack Growth' })
