...
export function Top10EvolutionView({ data }: Props) {
    ...
    const { frames, artistColors } = useMemo(() => {
        const uniqueYears = Array.from(
            new Set(data.map((d) => d.stream_year))
        ).sort((a, b) => a - b)
        ...
        const dataByYear = new Map<number, typeof data>()
        for (const row of data) {
            if (!dataByYear.has(row.stream_year)) {
                dataByYear.set(row.stream_year, [])
            }
            dataByYear.get(row.stream_year)!.push(row)
        }

        for (const year of uniqueYears) {
            const yearEvents = dataByYear.get(year) || []
