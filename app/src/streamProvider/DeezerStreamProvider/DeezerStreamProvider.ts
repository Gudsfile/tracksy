...
export class DeezerStreamProvider extends StreamProvider<DeezerRawStreamRecord> {
    ...
    transform(rawData: DeezerRawStreamRecord[]): StreamRecord[] {
        return rawData.map((raw) => {
            const listeningTime = Number(raw['Listening Time'])
            const ms_played = isNaN(listeningTime) ? 0 : listeningTime * 1000
            return {
                track_uri: `isrc:${raw['ISRC']}`,
                ...
                platform_model: raw['Platform Model'],
            }
        })
    }
}
