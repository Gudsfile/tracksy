...
def test_write_zip_is_deterministic(tmp_path, streaming_record):
    ...
    write_zip(zip_path_1, files, base_zipped_folder="folder")
    write_zip(zip_path_2, files, base_zipped_folder="folder")
    # then - they should have the same sha (it is acceptable to say they are byte-for-byte identical)
    assert (
...
def test_spotify_writer_write_calls_helpers(
    mock_write_zip, mock_write_json, streaming_record, default_generation_config
):
    # given
    writer = SpotifyWriter(output_dir=Path("tmp_path"), config=default_generation_config)
